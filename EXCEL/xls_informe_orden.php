...
try {
    ...
    foreach ($data_resumen as $res) {
        if ($res['fabricado']) {
            ...
            $row++;
            $data_id_fab = ModeloInforme::mdlInformeOrdenResumenFab($res['id_producto']);

            foreach($data_id_fab as $fab ){
                $sheet->setCellValue('A' . $row, 'UTILIZADO EN FABRICACIÓN:');
                $sheet->setCellValue('B' . $row, $fab['descripcion']);
                $sheet->setCellValue('C' . $row, $fab['unidad']);
                $sheet->setCellValue('D' . $row, $fab['cantidad_salida']);
                $sheet->setCellValue('E' . $row, $fab['retorno']);
                $sheet->setCellValue('F' . $row, $fab['utilizado']);
                $sheet->getStyle('A' . $row . ':F' . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('F2DCDB');
                $sheet->getStyle('C' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle('D' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle('E' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle('F' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
